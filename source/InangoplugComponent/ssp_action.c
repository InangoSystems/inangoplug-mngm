...
#define  CCSP_DATAMODEL_XML_FILE           "TR-181-InangoplugComponent.xml"
    ...
    )
{
    /* Create component common data model object */
     ...
     g_pComponent_COMMON_inangoplugcomponent = (PCOMPONENT_COMMON_INANGOPLUGCOMPONENT)AnscAllocateMemory(sizeof(COMPONENT_COMMON_INANGOPLUGCOMPONENT));

    if ( ! g_pComponent_COMMON_inangoplugcomponent )
    {
        return ANSC_STATUS_RESOURCES;
    }

    ComponentCommonDmInit( g_pComponent_COMMON_inangoplugcomponent);

     g_pComponent_COMMON_inangoplugcomponent->Name     = AnscCloneString(CCSP_COMPONENT_NAME_INANGOPLUGCOMPONENT);
     g_pComponent_COMMON_inangoplugcomponent->Version  = 1;
     g_pComponent_COMMON_inangoplugcomponent->Author   = AnscCloneString("Your name");
    ...
    )
{
    ...
    char                            CrName[256];
     ...
     g_pComponent_COMMON_inangoplugcomponent->Health = CCSP_COMMON_COMPONENT_HEALTH_Yellow;
    ...
    returnStatus =
        pDslhCpeController->RegisterCcspDataModel
            (
                ...
                CCSP_COMPONENT_NAME_INANGOPLUGCOMPONENT,            /* Component Name    */
                CCSP_COMPONENT_VERSION_INANGOPLUGCOMPONENT,         /* Component Version */
                CCSP_COMPONENT_PATH_INANGOPLUGCOMPONENT,            /* Component Path    */
    ...
    if ( returnStatus == ANSC_STATUS_SUCCESS || returnStatus == CCSP_SUCCESS)
    {
        /* System is fully initialized */
         g_pComponent_COMMON_inangoplugcomponent->Health = CCSP_COMMON_COMPONENT_HEALTH_Green;
    }
    ...
    )
{
    ...
    if(  g_pComponent_COMMON_inangoplugcomponent == NULL)
    {
        return ANSC_STATUS_SUCCESS;
    }
    ...
    {
        _ansc_sprintf(CrName, "%s%s", g_Subsystem, CCSP_DBUS_INTERFACE_CR);
        _ansc_sprintf(CpName, "%s%s", g_Subsystem, CCSP_COMPONENT_NAME_INANGOPLUGCOMPONENT);
    }
    else
    {
        _ansc_sprintf(CrName, "%s", CCSP_DBUS_INTERFACE_CR);
        _ansc_sprintf(CpName, "%s", CCSP_COMPONENT_NAME_INANGOPLUGCOMPONENT);
    }
    ...
    if (  g_pComponent_COMMON_inangoplugcomponent ) AnscFreeMemory( g_pComponent_COMMON_inangoplugcomponent);

     g_pComponent_COMMON_inangoplugcomponent = NULL;
    pSsdCcdIf                = NULL;
...
{
    return  g_pComponent_COMMON_inangoplugcomponent->Name;
}
...
{
    return  g_pComponent_COMMON_inangoplugcomponent->Version;
}
...
{
    return  g_pComponent_COMMON_inangoplugcomponent->Author;
}
...
{
    return  g_pComponent_COMMON_inangoplugcomponent->Health;
}
...
{
    return  g_pComponent_COMMON_inangoplugcomponent->State;
}
...
{
    return  g_pComponent_COMMON_inangoplugcomponent->LogEnable;
}
...
{
    if( g_pComponent_COMMON_inangoplugcomponent->LogEnable == bEnabled) return ANSC_STATUS_SUCCESS;
     g_pComponent_COMMON_inangoplugcomponent->LogEnable = bEnabled;
    if(bEnabled) g_iTraceLevel = (INT)  g_pComponent_COMMON_inangoplugcomponent->LogLevel;
...
{
    return  g_pComponent_COMMON_inangoplugcomponent->LogLevel;
}
...
{
    if( g_pComponent_COMMON_inangoplugcomponent->LogLevel == LogLevel) return ANSC_STATUS_SUCCESS;
     g_pComponent_COMMON_inangoplugcomponent->LogLevel = LogLevel;
    if( g_pComponent_COMMON_inangoplugcomponent->LogEnable) g_iTraceLevel = (INT)  g_pComponent_COMMON_inangoplugcomponent->LogLevel;
...
{
    return  g_pComponent_COMMON_inangoplugcomponent->MemMinUsage;
}
    ...
    )
{
    ...
    size = AnscGetComponentMemorySize(CCSP_COMPONENT_NAME_INANGOPLUGCOMPONENT);
    if (size == -1 )
        size = 0;
    ...
    )
{
    ...
    /* AnscSetTraceLevel((INT) g_pComponent_COMMON_inangoplugcomponent->LogLevel); */
